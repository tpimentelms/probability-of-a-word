...
class GPTBaseModel():
    ...
    def get_bow_markers(self):
        vocab = self.tokenizer.get_vocab()
        n_logits = self.model.embed_out.out_features
        
        start_of_words = set(idx for word, idx in vocab.items() if word[0] == self.start_of_word_symbol)
        self.bow_mask = torch.zeros(n_logits)
        self.bow_mask[torch.LongTensor(list(start_of_words))] = 1

        start_of_punctuation = set(idx for word, idx in vocab.items() if word[0] in string.punctuation)
        self.punctuation_mask = torch.zeros(n_logits)
        self.punctuation_mask[torch.LongTensor(list(start_of_punctuation))] = 1
        self.punctuation_mask[self.tokenizer.eos_token_id] = 0
        ...
        self.midword_mask = torch.ones(n_logits) - self.bow_mask - self.punctuation_mask - self.useless_mask
    ...
    def get_predictions(self, sentence, BOS=True):
        return self.get_models_output(sentence, BOS=BOS)

    def get_models_output(self, sentence, BOS=True):
        with torch.no_grad():
            all_results = {metric: torch.tensor([], device=self.device) for metric in self.metrics.keys()}
            offset_mapping = []
            start_ind = 0
    ...
    def _get_bow_fix(self, logits, _, __, ___):
        ...
        # bow_fix = - np.log(((self.bow_mask + self.punctuation_mask) * probs).sum(-1))
        bow_fix = - np.log(((self.bow_mask + self.eos_mask) * probs).sum(-1))

        return bow_fix.view(-1).cpu().numpy()
    
    def _get_bos_fix(self, logits, _, __, ___):
        probs = F.softmax(logits, dim=-1)
        bos_fix = - np.log(((self.midword_mask + self.punctuation_mask + self.eos_mask) * probs).sum(-1))

        return bos_fix.view(-1).cpu().numpy()
    
    def _get_tokens(self, _, __, ___, input):
        return np.array(self.tokenizer.convert_ids_to_tokens(input[0]))[1:]
